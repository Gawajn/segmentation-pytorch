...
def main():
    ...
    if args.test_input == [] and args.folds > 1:
        ...
        for ind, x in enumerate(kf.split(train, None)):
            ...
            train_dataset = MaskDataset(train_fold, map, transform=compose([base_line_transform()]),
                                        scale_area=args.scale_area,
                                        crop=args.crop_train, crop_x=args.crop_x_train, crop_y=args.crop_y_train)
            test_dataset = MaskDataset(test_fold, map, transform=compose([base_line_transform()]),
                                       scale_area=args.scale_area,
        ...
        train_dataset = MaskDataset(train, map, transform=compose([base_line_transform()]),
                                   scale_area=args.scale_area,
                                   crop=args.crop_train, crop_x=args.crop_x_train, crop_y=args.crop_y_train)
        test_dataset = MaskDataset(test, map, transform=compose([base_line_transform()]),
                                  scale_area=args.scale_area,
                                  crop=args.crop_val, crop_x=args.crop_x_val, crop_y=args.crop_y_val)
